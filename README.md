...
| features                   | `{feature1};{feature2};[...]` | Semicolon-separated list of features to enable. Options: `connectrpc`, `google.api.http`, `google.api.field_behavior`, `twirp`, `gnostic`, `protovalidate`; Default: `connectrpc;google.api.http;google.api.field_behavior;gnostic;protovalidate`. If this option is used, only the specified features will be enabled. |
...
- `google.api.field_behavior`: Enables support for `google.api.field_behavior` annotations (e.g., `REQUIRED`, `OUTPUT_ONLY`).
...
- **Enable ConnectRPC and Google API Field Behavior support:**
  ```yaml
  opt:
    - features=connectrpc;google.api.field_behavior
  ```
...
- **Enable all features:**
  ```yaml
  opt:
    - features=connectrpc;google.api.http;google.api.field_behavior;twirp;gnostic;protovalidate
  ```
